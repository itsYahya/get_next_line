...
/*   Updated: 2021/12/14 01:02:46 by yel-mrab         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (0);
	check = 0;
	line = get_the_line(0, reminder[fd], &index, &check);
	reminder[fd] = get_the_rest(reminder[fd], index, &check);
	if (check == -42)
		return (ft_free(line));
	if (check)
		return (line);
	...
	if (data.ko)
		return (0);
	reminder[fd] = get_the_rest(data.table, index, &check);
	if (check == -42)
